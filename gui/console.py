...
class Console():
	...
	class Script():
		...
		def try_update_button_states(self):
			...
			avaliable = self.console.option_browser.get_let_actions_list()

			register = {}

			for let in avaliable:
				register[let] = False
		...
		def modify(self, *args, **kwargs):
			"""
			try to find object specified by identity of x first argumets
		...
		def insert_or_modify(self, force_new_instance, *args, **kwargs):
			...
			"""
			# wheteher to insert new instances on top or bottom of script
		...
		def load_highlight(self):
			simulator_func = ["let", "Ball", "Force", "push", "rand", "Sound", "V", "Vector3", "active_objects", "Wall", "Box"]
			python_tfn = ["None", "True", "False", "-?\d+\.\d+", "-?\d+"]
			python_statements = ["nonlocal", "and", "as", "assert", "break", "class", "continue", "def", "del", "elif",
			...
			self.highlight.add_words(simulator_func, "sim_func", foreground="green")
